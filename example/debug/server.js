...
const source_path = path.resolve(__dirname + "/../TestApplication/qml")

// -----------------------------------
// HTTP SERVER - serve up qml folder
// -----------------------------------
const serve = serveStatic(source_path, {'index': ['main.qml']})
...
server.listen(http_port, console.log.bind(null, `http server listening on port ${http_port}`))

// ----------------------------------------------------------------
// Websocket server - used to send reload notification to clients
// ----------------------------------------------------------------
const WebSocketServer = require('ws').Server
, wss = new WebSocketServer({ host: "0.0.0.0", port: ws_port }, console.log.bind(null, `websocker server listening on port ${ws_port}`))
